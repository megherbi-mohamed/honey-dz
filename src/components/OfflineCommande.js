...
const OfflineCommande = () => {
    ...
    let formData = {commande:form, facture: id ? setFacture(product,true) : setFacture(items,false)}
    
    const handleSubmit = (e) => {
        e.preventDefault();
        if (form.email === '') {alert.error('Enter your email address please',{ timeout: 4000})}
        else if (form.firstname === '') {alert.error('Enter your firstname please',{ timeout: 4000})}
            ...
            if (id) {
                dispatch(insertOfflineProductCommande(formData,navigate))
            }
            else{
                dispatch(insertOfflineCartCommande(formData,navigate,emptyCart))
            }
        }
    }
                ...
                <div className='w-[100vw] md:w-[50vw] md:ml-[10vw] md:p-[56px_66px_21px_5vw]'>
                    ...
                    <div className='p-[20px] md:p-0 text-left'>
                        ...
                        <form onSubmit={handleSubmit}>
                            <input onChange={handleChange} type="email" name="email" value={form.email || ''} autoComplete="off" placeholder='Email address' className='mt-[5px] px-[12px] py-[10px] w-full border border-[#dbdbdb] rounded-[5px] outline-0 transition-[border] duration-400 ease-in-out focus:border-[#bd8c27]' />
...
export default OfflineCommande