...
import { insertOnlineCartCommande, insertOnlineProductCommande } from '../actions/commande';
import { getUserAddresses } from '../actions/address';
import { getStates } from '../actions/countryState';
import { getProduct } from '../actions/products';

const OnlineCommande = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (form.firstname === '') {alert.error('Enter your firstname please',{ timeout: 4000})}
            ...
            if (id) {
                dispatch(insertOnlineProductCommande(formData,navigate))
            }
            else{
                dispatch(insertOnlineCartCommande(formData,navigate,emptyCart))
            }
        }
    }
