...
const Products = () => {
    ...
    useEffect(() => {
        dispatch(getProduct(name.replace(/-/g, ' ')));
        setScroll()
    },[])
    ...
    const addProduct = (product) => {
        ...
        updateItem(product.id, { size: size})
    ...
    let borderButton = {xsmall:'!border-black',small:'',medium:'',large:'',xlarge:''}
    const [border, setborder] = useState(borderButton)
    ...
    return (
        <div className="w-full lg:max-w-[1200px] mx-auto my-[56px] md:my-[60px] ">
            {!product.length ? <p className=''>Loading</p> : (
                <>
                    <div className="mt-[60px] px-[12px] py-[30px] hidden md:block">
                        <div className="flex items-center">
                            ...
                            <span className="p-[16px] text-[.9rem] capitalize">{product[0].category}</span>
                        </div>
                    </div>
                        ...
                        <div className='w-full md:px-4 mt-[20px] md:mt-0'>
                            ...
                            <div className="flex items-center md:items-end flex-wrap md:flex-nowrap">
                                ...
                                <div className='flex flex-1 md:flex-none items-center bg-[#f1f1f1] rounded-[3px] w-[8rem] md:mr-[1.25rem]'>
                                    ...
                                    <input className="flex-1 md:flex-none py-[4px] w-[40px] text-center bg-[#f1f1f1]" type="text" value={quantity.val} />
                                    <button className="w-[46px] h-[46px] text-[19px]" onClick={()=>setquantityVal('+')}>+</button>
                                </div>
                        ...
                        <div className="w-full lg:max-w-[calc(1100px+calc(35px/1.25)*2)] px-0 md:px-[16px] lg:px-[28px] mx-auto flex justify-between items-center">
                            ...
                            <div className="flex md:items-center w-full md:w-auto justify-between md:justify-auto">
                                <div class="select-wrapper">
                                    <select name="" id="" className="w-[calc(100vw-177px)] md:w-[250px] p-[9.5px] pr-[30px] outline-none border-[1.5px] bg-transparent border-gray rounded">
                                        <option value="Taupe / X-Small">{product[0].color} / Taupe / X-Small - {product[0].prix}</option>
                                        <option value="Taupe / Small">{product[0].color} / Taupe / Small - {product[0].prix}</option>
                                        <option value="Taupe / Medium">{product[0].color} / Taupe / Medium - {product[0].prix}</option>
                                        <option value="Taupe / Large">{product[0].color} / Taupe / Large - {product[0].prix}</option>
                                        <option value="Taupe / X-Large">{product[0].color} / Taupe / X-Large - {product[0].prix}</option>
                                    </select>
                                </div>
                                <div className='md:w-[130px] ml-[20px] hidden md:flex border-[1.5px] border-gray rounded items-center'>
                                    <button className="w-[46px] h-[46px] text-[19px]" onClick={()=>setquantityVal('-')}>-</button>
                                    <input className="py-[4px] w-[40px] text-center bg-transparent" type="text" value={quantity.val} />
                                    <button className="w-[46px] h-[46px] text-[19px]" onClick={()=>setquantityVal('+')}>+</button>
                                </div>
                                <button type="button" onClick={()=>addProduct(product[0])} className="md:ml-[20px] px-[32px] py-[12px] bg-[#bd8c27] text-white text-sm block rounded-[5px] transition-[outline] duration-600 ease-in-out outline outline-0 outline-[#bd8c27] hover:outline-[3px]">Add to cart</button>
                            </div>
                        </div>
                    </div>
                </>
